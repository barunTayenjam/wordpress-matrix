( function( $ ){
    $( document ).ready( function(){
      $( '.eco-ngo-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( eco_ngo_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'eco_ngo_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
